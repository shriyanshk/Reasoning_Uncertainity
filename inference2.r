...
library(data.table)
library(caret)
fileVariable <- read.csv(file="C:/Users/Jani/Documents/Reasoning/ProjectSources/data/LidarData/LIDARNoMissingBeforeFreq.csv", 
                         header = TRUE, sep = ",")
for(i in 1:ncol(fileVariable)){
  fileVariable[,i] <- mapvalues(str, from = c(1,2,3,4,5), to = c("A", "B", "C", "D", "E"))
