...
for(i in 1:ncol(fileVariable)){
  #fileVariable[,i] <- mapvalues(fileVariable[,i], from = c(1,2,3,4,5), to = c("A", "B", "C", "D", "E"))
  fileVariable[,i] <- as.factor(fileVariable[,i])
 # hist(x=fileVariable[,i], breaks=6, freq=TRUE, labels=TRUE, main=colnames(fileVariable[i]))
}
#bn.iamb <- iamb(fileVariable, test = "mc-mi", max.iter=15, strict = FALSE)
#bn.iamb <- tabu(fileVariable, score="aic", max.iter = 60)
#adjmat <- amat(bn.iamb)
#print(adjmat)
...
inferrunner <- function(trainset, testset, runnum){
  ...
  bn.tabu <- tabu(fileVariable, score="aic", max.iter = 60)
  adjmat <- amat(bn.tabu)
  g2 <- graph_from_adjacency_matrix(adjmat)
  plot(g2)
  bn.plot <- moralizeMAT(adjmat)
